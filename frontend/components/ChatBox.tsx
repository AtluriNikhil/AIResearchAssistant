...
export default function ChatBox() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Auto-select the newly uploaded document using the backend doc_id.
      if (res.data.doc_id) {
        setSelectedDocIds(prev => Array.from(new Set([...prev, res.data.doc_id])));
      }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const assistantMessage: Message = {
        role: "assistant",
        content: res.data.answer || "The backend returned an empty answer. Check the backend terminal/logs for the model error."
      };
