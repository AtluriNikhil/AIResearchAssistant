import os
import config  # noqa: F401 - loads environment variables from the project .env
...
async def upload_file_v2(file: UploadFile = File(...)):
    ...
    try:
        ...
        doc_id = multi_doc_store.get_safe_doc_id(filename)
...
async def ask(req: AskRequest):
    ...
    api_logger.info(f"Invoking LLM - Provider: {provider}, Model: {model}")
    answer = call_llm([{"role": "user", "content": prompt}], model=model)
    api_logger.info(f"LLM response received | Answer length: {len(answer)} chars")
