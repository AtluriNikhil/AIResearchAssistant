...
SUGGESTIONS: [Stick to context OR add missing context info]"""
        ...
        try:
            ...
            agent_logger.info(
                f"{self.name}: Invoking LLM - Provider: {provider}, Model: {model}, Temperature: 0.4"
            )
            ...
            agent_logger.info(f"{self.name}: LLM response received | Response length: {len(critique)} chars")
