...
Your answer (strictly from context only):"""
        ...
        try:
            ...
            agent_logger.info(
                f"{self.name}: Invoking LLM - Provider: {provider}, Model: {model}, Temperature: 0.2"
            )
            ...
            agent_logger.info(f"{self.name}: LLM response received | Response length: {len(summary)} chars")
