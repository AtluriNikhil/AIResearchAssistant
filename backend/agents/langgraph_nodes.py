...
def critic_node(state: AgentState) -> AgentState:
    ...
    if state.get("status") == "error":
        workflow_log.append("[3/4] Skipped - Previous step failed")
        return {
            **state,
            "critique": "Critique skipped because summarization failed",
            "has_gaps": False,
            "suggestions": [],
            "workflow_log": workflow_log,
...
def skip_editor_node(state: AgentState) -> AgentState:
    ...
    workflow_log = state.get("workflow_log", [])
    if state.get("status") == "error":
        workflow_log.append("[4/4] Skipped - Returning workflow error")
        return {
            **state,
            "final_answer": state.get("final_answer") or state.get("error_message", "Workflow failed"),
